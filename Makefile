...
.PHONY: build import deploy seed status port-forward port-forward-api port-forward-rabbitmq port-forward-postgres port-forward-cache port-forward-all stop-port-forward cluster-up down cluster-down reset reset-all all logs
...
port-forward-all:
	mkdir -p .tmp
	(kubectl port-forward svc/api 8000:8000 -n $(NS) > .tmp/api-port-forward.log 2>&1 & echo $$! > .tmp/api-port-forward.pid)
	(kubectl port-forward svc/rabbitmq 15672:15672 -n $(NS) > .tmp/rabbitmq-port-forward.log 2>&1 & echo $$! > .tmp/rabbitmq-port-forward.pid)
...
all: stop-port-forward cluster-up wait-cluster build import deploy wait-infra migrate seed status port-forward-all

reset: down all

reset-all: stop-port-forward cluster-down cluster-up wait-cluster build import deploy wait-infra migrate seed status port-forward-all